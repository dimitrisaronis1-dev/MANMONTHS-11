...
def process_excel(input_bytes: bytes, input_filename: str = "") -> Tuple[bytes, str]:
    ...
    for i in range(2, last_row_cv + 1):
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        model.row_cv_aa[r] = int(row.get("cv_aa", idx))
        ...
        ws.cell(r, 1).value = model.row_cv_aa.get(r, None)
        ws.cell(r, 1).border = thin_border

        ws.cell(r, 2).value = f"=VLOOKUP(A{r},CV!$A$2:$B${last_row_cv},2,FALSE)"
        ws.cell(r, 2).border = thin_border

        ws.cell(r, 3).value = f"=VLOOKUP(A{r},CV!$A$2:$C${last_row_cv},3,FALSE)"
    ...
    ws["A6"] = f"=MATCH(C6,CV!$C$2:$C${last_row_cv},0)"
